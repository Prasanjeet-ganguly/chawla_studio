...
function initGallery() {
  ...
  // Uniform grid slot for all photos
  const UNIFORM_SLOT = { className: '', sizes: '(min-width: 1024px) 30vw, (min-width: 640px) 46vw, 92vw' };
  ...
  function renderPhotoGrid(ids) {
    ...
    ids.forEach((id, index) => {
      const el = makePhotoEl(id, UNIFORM_SLOT.sizes, UNIFORM_SLOT.className, index);
...
// Hero 01-04 slide counter
function initHeroCounter() {
  const counterItems = document.querySelectorAll('.hero-counter-item');
  const heroImages = document.querySelectorAll('.hero-image');
  if (!counterItems.length || !heroImages.length) return;
  counterItems.forEach((item) => {
    const btn = item.querySelector('.hero-counter-btn');
    if (!btn) return;
    btn.addEventListener('click', () => {
      const idx = parseInt(item.dataset.slide);
      counterItems.forEach(i => i.classList.toggle('active', parseInt(i.dataset.slide) === idx));
      heroImages.forEach(img => {
        const active = parseInt(img.dataset.slide) === idx;
        img.classList.toggle('active', active);
        if (active) img.loading = 'eager'; else img.loading = 'lazy';
      });
    });
  });
}
...
function initFeaturedFilters() {
  document.querySelectorAll('.open-story-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      tabs.forEach(t => {
        if (t.dataset.category === 'all' || t.dataset.category === cat) {
          t.classList.add('active');
        } else {
          t.classList.remove('active');
        }
      });
      // Trigger gallery render with category
      const galleryTab = document.querySelector('.filter-tab[data-group="photography"]');
      if (galleryTab) galleryTab.click();
      const catTab = document.querySelector('.category-tab[data-category="' + cat + '"]');
